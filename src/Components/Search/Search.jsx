...
function Search({ setIsOpen }) {
  ...
  return (
        <>
        <form
            className={`flex items-center w-full max-w-sm border-b-2 border-secondary ${
            check ? "animate-shake border-red-500" : ""
            }`}
            onSubmit={handleSubmit}
        >
            <input
            type="text"
            placeholder={`${check?"This section can not be empty":"Search news by topic..."}`}
            ...
            <svg
                ...
                check ? "text-red-500 scale-110" : "text-secondary"
                }`}
            >
                <path
                strokeLinecap="round"
